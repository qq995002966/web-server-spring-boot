    ...
    return this.each(function () {
      ...
      for(i in opts.sliders){
        if (opts.sliders[i].link){
          dom.imageList += '<div class="image-box" style="width:'+ GetPercent(1,opts.sliders.length) +'"><a href="' + opts.sliders[i].link + '"><img src="' + opts.sliders[i].image + '"/></a></div>'
        }else {
      ...
      function slider(add) {
        root.find('.slider-controllers').children('span').unbind();
        var targetList = root.find('.image-list');
        var targetListLeft = Number(targetList.css('left').slice(0, targetList.css('left').length-2));
