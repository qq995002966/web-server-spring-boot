...
var F_Person = {
    _getInfo:function(){
        ...
        var nick = person.nick_name ? (person.nick_name.indexOf('*TG') == 0 ? '' : person.nick_name) : '';
        $('input[name="nick"]').val(nick);

        var mobile = person.mobile ? person.mobile.substr(0,3)+'*****'+person.mobile.substr(-3,3) : "<button onclick='openBind();' class='btn btn-default'>点击绑定手机号</button>";
