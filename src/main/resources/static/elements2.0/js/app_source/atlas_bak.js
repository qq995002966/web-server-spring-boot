    ...
    _init:function (gameId) {
        ...
        if(B_User._isDemoUser()){
            ...
            if ($_GET.g && !isNaN($_GET.g)) {
                ...
                B_Game._setLast(M_Init._gameId, 'outsideAtlas');
            }else{
                if(gameId){
                    M_Init._gameId = gameId;
                    B_Game._setLast(M_Init._gameId, 'outsideAtlas');
                }else{
                    M_Init._gameId = M_Init._getGameId('outsideAtlas');
                }
            }
...
var F_Atlas_Info = {
    ...
    _search:function (keywords) {
        M_Outside._redirectCenter(B_Common._encodeUrl(keywords));
    },
    _getInfo:function(){
        if (M_Init._dataCache['data'] && M_Init._dataCache['tag']) {
            F_Atlas_Info._htmlFormat(M_Init._dataCache['data'],M_Init._dataCache['tag']);
        } else {
            var dom = $('#canvas');
            B_Port._ajax('atlasProjectGraphList', 'get', true, 'project_id=' + M_Init._gameId, function() {
                dom.html(B_Pre._loading());
            }, function() {
                dom.html('');
            }, function(data, msg) {
                if (data && !B_Common._checkObjectIsEmpty(data)) {
                    M_Init._dataCache['data'] = data.child;
                    M_Init._dataCache['tag'] = data.tag_list;
    ...
    _htmlCenter: function(tag) {
        var str = '';
        var tagStr = F_Atlas_Info._htmlTag(M_Init._gameId, tag);
        str += '<div class="pop" data-i="' + M_Init._gameId + '">';
        str += '<ul class="inner-circle">';
        str += '<li><a onclick="F_Atlas_Info._changeGame('+ M_Init._gameId + ');">' + '查看图谱' + '</a></li>';
        str += '<li><a onclick="M_Outside._redirectLight('+ M_Init._gameId + ');">' + '灯塔透视' + '</a></li>';
        str += '</ul>';
        str += '<ul class="style-list" id="tag' + M_Init._gameId + '">' + tagStr;
        str += '</ul>';
        str += '</div>';

        $('#canvas').append('<div id="center" class="arc"><b class="img-title">' + B_Game._getGame([M_Init._gameId],1)[M_Init._gameId][1] + '</b><img src="' + B_Game._imgUrl(M_Init._gameId) + '">'+str+'</div>');
        var center = $('#center');
    ...
    _htmlGame: function(cycle, data) {
        ...
        str += '<li><a onclick="M_Outside._redirectLight('+ data.gameId + ');">' + '灯塔透视' + '</a></li>';
        str += '</ul>';
