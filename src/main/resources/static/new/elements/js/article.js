...
$(function(){
    ...
    switch(controller){
        case 'article':
            var typeArr = ['classify','top','hot','game'];
            if($_GET.t && ($.inArray($_GET.t,typeArr) > -1)){
                ...
                if($_GET.e){
                    postData.push('data_date_start='+$_GET.e);
                ...
                switch($_GET.t){
                    ...
                    case 'top':
                        isIcon = false;
                        title = '今日头条';
                        postData.push('data_date_start='+dateEnd);
...
var F_List = {
    _getList:function(postData,queryData,isIcon,page){
        G_Port._ajax('articleList','get',true,postData.join('&'),function(){
                    ...
                    $('.page-list span').each(function(){
                        ...
                        $(this).click(function(){
                            ...
                            if(isJump){
                                 ...
                                 */
                                //ajax异步
                                postData['index'] = (page-1)*G_Page._size
...
var F_Classify = {
    ...
    _getPort:function(index,data){
        ...
        G_Port._ajax('articleList','get',true,'main_class='+encodeURIComponent(data)+'&sub_class=&index=0&limit=5',function(){
    ...
    _htmlList:function(data){
        ...
        for(var i=0;i<data.length;i++){
            ...
            str += '<li title="'+data[i].source.title+'"><a href="'+G_Jump._getUrl('detail')+'?t='+encodeURIComponent(data[i].source.title)+'" target="_blank"><span class="guidelines-date">'+data[i].source.post_date+'</span><span class="guide-title">'+articleTitle+'</span></a></li>';
        }
        return str;
    }
}
var F_Hot = {
    _getRelation:function(data){
        G_Port._ajax('articleRelation','get',true,'title='+encodeURIComponent(data),function(){
                ...
                if(data.related_article_list && data.related_article_list.length>0){
                    $('#bs_hot_relation').html(F_Hot._htmlRelation(data.related_article_list));
                }
            },function(data,msg,code){
    ...
    _getHotToday:function(){
        G_Port._ajax('articleList','get',true,'data_date_start='+dateEnd+'&data_date_end='+dateEnd+'&order_by_field=score&order_type=desc&index=0&limit=5',function(){
    ...
    _htmlRelation:function(data){
        ...
        for(var i=0;i<data.length;i++){
            if(i>4)break;
            //var articleTitle = (data[i].title.length > 13) ? data[i].title.substr(0,13)+'..' : data[i].title;
            var articleTitle = data[i].title;
            str += '<li title="'+data[i].title+'"><a href="'+G_Jump._getUrl('detail')+'?t='+encodeURIComponent(data[i].title)+'" target="_blank"><span class="guidelines-date">'+data[i].data_date+'</span><span class="guide-title">'+articleTitle+'</span></a></li>';
        }
        return str;
    }
}
