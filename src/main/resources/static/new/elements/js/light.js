...
$(function(){
    ...
    F_Info._getGame();
    F_Info._getArticle();
    dataChoose._section({'autoCommit':true,'todayValid':false},1,dateBegin,dateEnd,function(begin,end){
    ...
    dataChoose._section({'autoCommit':true,'todayValid':false},2,dateBegin,dateEnd,function(begin,end){
    ...
    dataChoose._section({'autoCommit':true,'todayValid':false},3,dateBegin,dateEnd,function(begin,end){
                ...
                switch(type){
                    case '正面':
                        $('.lt_fmPl .lt_fmPlWord a').each(function(){
                            $(this).addClass('lt_fmPlWPos');
                        });
                        break;
                    case '负面':
                        $('.lt_fmPl .lt_fmPlWord a').each(function(){
                            $(this).addClass('lt_fmPlWNeg');
                        });
                        break;
                }
            }
    ...
    _htmlRank:function(data){
        ...
        for(var i=0;i<data.length;i++){
            ...
            if(data[i].project_id){
                ...
                shade = '<div class="lt_notin">该游戏暂未收入ThinkingGame数据库</div>';
            }
            str += '\
                    <li>'+rank+'\
...
var F_Channel = {
    ...
    _getInfo:function(){
        ...
        G_Port._ajax('appChannelInfo','get',true,'project_id='+G_GameId,function(){
    ...
    _getChart:function(){
        var begin = F_Channel._getDateBegin();
        var end = F_Channel._getDateEnd();
        G_Port._ajax('appChannelChart','post',true,'data_date_start='+begin+'&data_date_end='+end+'&project_id='+G_GameId,null,null,function(data,msg){
            if(data.channel_rating_distri && data.channel_rating_distri.length > 0){
                var channelPortData = data.channel_rating_distri;
                var channelData = {};
                for(var i=0;i<channelPortData.length;i++){
                    if(channelPortData[i]){
                ...
                if(channelData){
                    $.each(channelData,function(key,value){
                        ...
                        $('bs_yxqd_'+key).html('');
                        getEChart('line','bs_yxqd_'+key,chartData);
                    });
                }
            }
        },null)
    ...
    _htmlBase:function(data){
        ...
        for(var i=0;i<data.length;i++){
            var pcount = 0;
            str += '<li><div class="lt_fmQdLogo" data-t="'+data[i].source_type+'"><img src="http://image.thinkinggame.cn/img/source/'+data[i].source_type+'_s.png"></div>';
...
var F_Forum = {
    ...
    _getUseless:function(type){
        ...
        G_Port._ajax('forumUseless','get',true,'data_date_start='+begin+'&data_date_end='+end+'&project_id='+G_GameId,function(){
                ...
                if(data.useless_distri && data.useless_distri.length>0){
    ...
    _getTopic:function(){
        if(F_Forum._buffData.topic){
            F_Forum.__htmlTopic(F_Forum._buffData.topic);
            return;
        }
        var begin = F_Forum._getDateBegin();
        var end = F_Forum._getDateEnd();
        G_Port._ajax('forumTopic','get',true,'data_date_start='+begin+'&data_date_end='+end+'&project_id='+G_GameId,function(){
                ...
                if(data.topic_distri && data.topic_distri.length>0){
                    F_Forum.__htmlTopic(data.topic_distri);
                }
            },function(data,msg,code){
                $('.lt_ltfx').html(G_Pre._empty(msg));
            }
    ...
    __htmlTopic:function(data){
        ...
        for(var i=0;i<data.length;i++){
            if(i>14)break;
            diff = parseInt(i/3)+1;
            str += '<div class="lt_ltfxR'+diff+' lt_ltfxRP'+(i+1)+'">';
    ...
    _chartKeywords:function(data){
        ...
        getEChart('line','bs_ltfx',chartData);
    },
    _chartUseless:function(data,type){
        ...
        for(var i = 0;i<data.length;i++){
            if(data[i].detail_classify_distri && data[i].detail_classify_distri.length>0){
                ...
                var inside_number = (data[i].useless_rate*100).toFixed(3);
                var inside_source = data[i].show_forum_name+' '+inside_number+'%';
                ...
                if(type == data[i].show_forum_name){
                    for(var d=0;d<data[i].detail_classify_distri.length;d++){
                        var outside_number = (data[i].detail_classify_distri[d].useless_rate*100).toFixed(3);
                        var outside_source = '('+data[i].show_forum_name+')'+data[i].detail_classify_distri[d].useless_classify+' '+outside_number+'%';
                ...
                $.each(data,function(key,dataArr){
                    ...
                    if(dataArr){
                        for(var i=0;i<dataArr.length;i++){
                            var date = G_Date._getByDiff(dataArr[i][0],0);
                            chartDataPre.word[key].date.push(date);
                            chartDataPre.word[key].number.push(dataArr[i][1]);
                        }
                    }
                });
                var color = ['#83B935','#ADD633','#FFD834','#FFB234','#FF8B5A'];
                var i = 1;
...
var F_Insight = {
    _getInfo:function(){
        ...
        G_Port._ajax('insight','get',true,'data_date_start='+begin+'&data_date_end='+end+'&project_id='+G_GameId,function(){
            ...
            function(data,msg){
                ...
                var rate_red = negative_rate > 0 ? (negative_rate/5).toFixed(0) : 0;
                if(rate_red == 0 && negative_rate >0)rate_red = 1;
                var rate_blue = other_rate > 0 ? (other_rate/5).toFixed(0) : 0;
                if(rate_blue == 0 && other_rate >0){
                    rate_blue = 1;
                    if(rate_red == 20)rate_red -= 1;
                }
                var rate_green = 20 - rate_red - rate_blue;
                if(rate_green <= 0){
                    if(rate_green < 0){
                        if(rate_red > rate_blue){
                            rate_red -= 1;
                        }else{
                            rate_blue -= 1;
                        }
                ...
                if(rate_green == 20 && positive_rate == 0)rate_green = 0;
                ...
                var word = {'positive_total':0,'positive':{},'negative_total':0,'negative':{},'other_total':0,'other':{}};
                var wordGet = {};
                if(data.feedback_class_distri){
                    $.each(data.feedback_class_distri,function(key,wordClassify){
                        ...
                        switch(key){
                            ...
                            case 1:
                                for(var i=0;i<wordClassify.length;i++){
                                    if(i==0)wordGet.positive =  wordClassify[i].lighttower_classify;;
                            ...
                            case -1:
                                for(var i=0;i<wordClassify.length;i++){
                                    if(i==0)wordGet.negative =  wordClassify[i].lighttower_classify;;
                                    word.negative_total += wordClassify[i].post_num;
                                    word.negative[wordClassify[i].lighttower_classify] = wordClassify[i].post_num;
                                }
                                break;
                        }
                    })
    ...
    _getRelation:function(begin,end,word,classify){
        ...
        G_Port._ajax('insightRelation','get',true,'data_date_start='+begin+'&data_date_end='+end+'&classify_sentiment='+classify+'&lighttower_classify='+encodeURIComponent(word)+'&project_id='+G_GameId,function(){
            ...
            if(data.feedback_tag_distri){
                ...
                var chartData2 = {};
                var color = ['#DD4A38','#E36E5F','#EB9389','#F1B6AF','#F7DAD8'];
    ...
    _htmlBase:function(type,data){
        ...
        switch(type){
            case 'curve':
                str = '<p>共<span class="c_colorG">'+data.total_num+'</span>份玩家反馈。负面反馈<span class="c_colorR">'+data.negative_num+'</span>份。正面反馈<span class="c_colorGS">'+data.positive_num+'</span>份。</p>\
            ...
            case 'percent':
                ...
                <div class="lt_fmPercent c_floatLeft">\
                    ...
                    <p><i class="lt_fmBlue c_img"></i>其他反馈<br>'+data.other_rate+'%</p>\
                        ...
                        case 'positive':
                            ...
                            $.each(wordArr,function(word,value){
                                if(i < 12){
                                    ...
                                    if(i == 1)positiveStr+= ' class="liOn"';
                                    positiveStr+= '><span>'+i+'</span>'+word+'<div class="lt_fmTsLine"><div class="lt_fmTsLnMask';
                                    if(i == 1)positiveStr+= ' lt_fmTsLnOn';
                        ...
                        case 'negative':
                            ...
                            $.each(wordArr,function(word,value){
                                if(i < 12){
                                    ...
                                    negativeStr+= '<li data-w="'+word+'"';
                                    if(i == 1)negativeStr+= ' class="liOn"';
                                    negativeStr+= '><span>'+i+'</span>'+word+'<div class="lt_fmTsLine"><div class="lt_fmTsLnMask';
                                    if(i == 1)negativeStr+= ' lt_fmTsLnOn';
                                    negativeStr+= '" style="width: '+percent+'%"></div></div></li>';
                                }
                                i++;
                            });
                            break;
                        case 'other':
                            ...
                            $.each(wordArr,function(word,value){
                                if(i < 12){
                                    ...
                                    if(i == 1)otherStr+= ' class="liOn"';
                                    otherStr+= '><span>'+i+'</span>'+word+'<div class="lt_fmTsLine"><div class="lt_fmTsLnMask';
                                    if(i == 1)otherStr+= ' lt_fmTsLnOn';
...
var F_Info = {
    _getGame:function(){
        G_Port._ajax('gameInfo','get',true,'project_id='+G_GameId,function(){
            ...
            if(data.game_type.indexOf('S') > -1){
                ...
                dataChoose._section({'autoCommit':true,'todayValid':false},4,dateBegin,dateEnd,function(begin,end){
                ...
                dataChoose._section({'autoCommit':true,'todayValid':false},5,dateBegin,dateEnd,function(begin,end){
                ...
                dataChoose._section({'autoCommit':true,'todayValid':false},6,dateBegin,dateEnd,function(begin,end){
            ...
            var competing = '';
            if(competings){
                $.each(competings,function(key,value){
                    var collectClass = 'lt_gmCtOff';
                    if(G_Game._checkCollect(parseInt(key)))collectClass = 'lt_gmCtOn';
            ...
            var infoStr = F_Info._htmlGame(logo,name,tag,type,data.author,data.distributor,data.release_date,data.overview,competing);
            var rankStr = F_Info._htmlRank(type,data.hot_rank,data.hot_rank_yesterday);
            $('.lt_info').html(infoStr);
            $('.lt_rank').html(rankStr);
    ...
    _htmlGame:function(logo,name,tag,type,author,distributor,date,overview,competing){
        ...
        var str = '\
            ...
            <div class="c_padding30">\
                ...
                <dl class="c_floatLeft">\
                    <dt><h1 class="c_floatLeft">'+name+'</h1>　<i class="c_img c_floatLeft"></i>　<a onclick="G_Jump._url(\'atlas\','+G_GameId+')" class="c_floatLeft">查看游戏图谱></a></dt>\
                    <div class="clearfix"></div>\
                    <dd><span>开发商：</span>'+author+'</dd>\
                    <dd><span>发行商：</span>'+distributor+'</dd>\
    ...
    _htmlRank:function(type,todayRank,yesterdayRank){
        ...
        var str = '\
            <div class="c_padding30">\
                <div class="lt_title c_marginB10">\
                <h2>'+type+'类舆论热度排名</h2>\
                </div>\
                第 <b>'+todayRank+'</b> 名\
                <p><span class="'+useClass+'">较前日'+diff+'</span></p>\
                <p>以上排名依据舆情指数、玩家活跃度等综合因素计算</p>\
            </div>';
...
$(function(){
    $('.data-center-quicker').on('click',function(isApp){
        var isEI = function() {
            return get_browser().name == 'MSIE' && get_browser().version < 10 ? true : false;
        };
        if($('.iframe-more-date-hide').length > 0) {
            var src = $('#iframeInsert iframe').attr('src');
            if(!src)$('#iframeInsert iframe').attr('src',G_Jump._getUrl('data')+'?t='+$(this).attr('data-t'));
            if (isEI()) {
                $('.more').show().children('iframe').removeClass('iframe-more-date-hide').addClass('iframe-more-date-ie');
            }else {
                $('.more').show().children('iframe').removeClass('iframe-more-date-hide').addClass('iframe-more-date');
            }
        }else {
            if(isEI()) {
                $('.more').children('iframe').removeClass('iframe-more-date-ie', 'iframe-more-date').addClass('iframe-more-date-hide');
            }else {
                $('.more').children('iframe').removeClass('iframe-more-date').addClass('iframe-more-date-hide');
            }
        }
        $('.data-center-quicker .arrow').toggleClass('arrow-op');
    });
});